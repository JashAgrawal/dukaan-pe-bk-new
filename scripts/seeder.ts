...
const createStoreCategories = async (): Promise<mongoose.Types.ObjectId[]> => {
  const categories = [
    { name: "Fashion", image: "/static/images/uploads/storeorproductMainImage.png" },
    { name: "Electronics", image: "/static/images/uploads/storeorproductMainImage.png" },
    { name: "Home & Kitchen", image: "/static/images/uploads/storeorproductMainImage.png" },
    { name: "Beauty & Personal Care", image: "/static/images/uploads/storeorproductMainImage.png" },
    { name: "Books & Stationery", image: "/static/images/uploads/storeorproductMainImage.png" },
    { name: "Sports & Fitness", image: "/static/images/uploads/storeorproductMainImage.png" },
    { name: "Toys & Games", image: "/static/images/uploads/storeorproductMainImage.png" },
    { name: "Grocery & Gourmet", image: "/static/images/uploads/storeorproductMainImage.png" },
    { name: "Health & Wellness", image: "/static/images/uploads/storeorproductMainImage.png" },
    { name: "Jewelry & Accessories", image: "/static/images/uploads/storeorproductMainImage.png" }
  ];
  ...
  try {
    ...
    return createdCategories.map(category => category._id);
  } catch (error) {
    logger.error(`Error creating store categories: ${error instanceof Error ? error.message : String(error)}`);
    return [];
  }
};

// Create product categories
const createProductCategories = async (): Promise<mongoose.Types.ObjectId[]> => {
  const categories = [
    { name: "Clothing", image: "/static/images/uploads/storeorproductMainImage.png" },
    { name: "Smartphones", image: "/static/images/uploads/storeorproductMainImage.png" },
    { name: "Laptops", image: "/static/images/uploads/storeorproductMainImage.png" },
    { name: "Furniture", image: "/static/images/uploads/storeorproductMainImage.png" },
    { name: "Skincare", image: "/static/images/uploads/storeorproductMainImage.png" },
    { name: "Fiction Books", image: "/static/images/uploads/storeorproductMainImage.png" },
    { name: "Fitness Equipment", image: "/static/images/uploads/storeorproductMainImage.png" },
    { name: "Board Games", image: "/static/images/uploads/storeorproductMainImage.png" },
    { name: "Organic Food", image: "/static/images/uploads/storeorproductMainImage.png" },
    { name: "Vitamins & Supplements", image: "/static/images/uploads/storeorproductMainImage.png" }
  ];
  ...
  try {
    ...
    logger.error(`Error creating product categories: ${error instanceof Error ? error.message : String(error)}`);
    return [];
  }
};

// Create users with seller role
const createUsers = async (count: number): Promise<mongoose.Types.ObjectId[]> => {
  const users = [];
  ...
  try {
    ...
    logger.error(`Error creating users: ${error instanceof Error ? error.message : String(error)}`);
    return [];
  }
};
...
const createStores = async (
  ...
  for (const userId of userIds) {
    const randomStoreCategoryId = storeCategoryIds[Math.floor(Math.random() * storeCategoryIds.length)];
    ...
    for (let i = 0; i < numProductCategories; i++) {
      const randomProductCategoryId = productCategoryIds[Math.floor(Math.random() * productCategoryIds.length)];
  ...
  try {
    ...
    return createdStores.map(store => store._id);
  } catch (error) {
    logger.error(`Error creating stores: ${error instanceof Error ? error.message : String(error)}`);
    return [];
  }
};
...
const createProducts = async (
  ...
  const productTypes: ProductTypeEnum[] = ["physical", "digital", "appointment"];
  let totalProducts = 0;
  ...
  for (const storeId of storeIds) {
    ...
    for (let i = 0; i < productsPerStore; i++) {
      ...
      const categoryPool = storeProductCategories.length > 0 ? storeProductCategories : productCategoryIds;
      ...
      if (hasSizeVariants) {
        ...
        for (let j = 0; j < numSizes; j++) {
          ...
          const variantSellingPrice = Math.round(variantPrice * (1 - discountPercentage / 100));
      ...
      if (hasVariants) {
        ...
        for (let j = 0; j < numVariants; j++) {
          const name = variantTypes[Math.floor(Math.random() * variantTypes.length)];
          const value = variantValues[name as keyof typeof variantValues][
            Math.floor(Math.random() * variantValues[name as keyof typeof variantValues].length)
          ];
          const variantPrice = price + faker.number.int({ min: 50, max: 500 });
          const variantSellingPrice = Math.round(variantPrice * (1 - discountPercentage / 100));
    ...
    try {
      ...
      logger.info(`Created ${createdProducts.length} products for store ${storeId}`);
    } catch (error) {
      logger.error(`Error creating products for store ${storeId}: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
...
const seedData = async (clearExisting: boolean = false): Promise<void> => {
  try {
    ...
    const storeIds = await createStores(userIds, storeCategoryIds, productCategoryIds);
