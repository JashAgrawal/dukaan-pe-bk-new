...
export interface ICatalogueProduct extends Document {
  name: string;
  description: string;
  mainImage: string;
  allImages: string[];
...
const catalogueProductSchema = new Schema<ICatalogueProduct>(
  {
    name: {
      type: String,
...
catalogueProductSchema.pre(/^find/, function (this: any) {
...
catalogueProductSchema.pre("save", function (next) {
...
catalogueProductSchema.index({
  name: "text",
  description: "text",
  tags: "text",
});

export const CatalogueProduct = mongoose.model<ICatalogueProduct>(
  "CatalogueProduct",
