 ...
 * @access Private
 */
export const getTrackingByOrderId = catchAsync(
 ...
 */
export const updateTrackingStatus = catchAsync(
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      // Find tracking
      const tracking = await DeliveryTracking.findById(trackingId).session(
        session
      );
      ...
      if (estimatedDeliveryDate)
        tracking.estimatedDeliveryDate = new Date(estimatedDeliveryDate);
 ...
 * @access Private (Seller/Admin)
 */
export const addTrackingDetails = catchAsync(
  async (req: Request, res: Response) => {
    const session = await mongoose.startSession();
    session.startTransaction();
    ...
    try {
      ...
      let tracking = await DeliveryTracking.findOne({ order: orderId }).session(
        session
      );
      ...
      if (!tracking) {
        ...
        order.deliveryTrackingId = tracking._id as any;
